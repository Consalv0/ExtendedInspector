...
{
    public class ArrayCollectionView : CollectionView
    {
        protected Array m_Value;
        ...
        public ArrayCollectionView( string label, Type collectionType, Type elementType, MemberInfo memberInfo, System.Func<object> get, System.Action<object> set, SerializedProperty property, Inspector inspector )
        ...
        protected void RemoveElement( int index )
        {
            ...
            else
            {
                ...
                Array newArray = Array.CreateInstance( m_ElementType, newSize );

                if ( m_Value != null )
                {
                    // Copy everything before the removed index
                    Array.Copy( m_Value, 0, newArray, 0, index );

                    // Copy everything after the removed index
                    Array.Copy( m_Value, index + 1, newArray, index, newSize - index );
                }
                m_Value = newArray;
        ...
        protected void AddElementAt( int index )
        {
            ...
            else
            {
                ...
                if ( m_Value != null && m_Size > 0 )
                {
                    ...
                    // Copy elements after the insertion point, shifted by one
                    Array.Copy( m_Value, index, newArray, index + 1, m_Size - index );
                }
        ...
        protected void UpdateCollectionSize()
        {
            ...
            for ( int i = oldSize; i < m_Size; i++ )
            {
                ...
                if ( m_Property == null )
                {
                    get = ( ) => m_Value.GetValue( index );
                    if ( m_Set == null ) set = null;
                    else
                    {
                        set = ( value ) => m_Value.SetValue( value, index );
                    }
                }
                else
                {
                    get = () => m_Get.Invoke() is Array array ? array.GetValue( index ) : null;
                    set = ( object value ) => { m_Value = m_Get.Invoke() as Array; m_Value?.SetValue( value, index ); m_Set?.Invoke( m_Value ); };
                }
        ...
        protected void MoveElementDown( int index )
        {
            ...
            {
                object temp = m_Value.GetValue( index );
                m_Value.SetValue( m_Value.GetValue( index + 1 ), index );
                m_Value.SetValue( temp, index + 1 );
            }
        }
        ...
        protected override void UpdateCollectionCache( )
        {
            if ( m_Property == null )
            {
                m_Value = GetFieldValue() as Array;
            }
